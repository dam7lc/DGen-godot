...
func _ready():
	...
	for j in range(cuantos): #For para cada habitacion
		...
		while(encontrar_lado):
			...
			room_x = rng.randf_range(8,15)
			room_z = rng.randf_range(8,15)
			...
			#await get_tree()._physics_process(delta)
			#await get_tree()._physics_process(delta)
		...
		if j != 0:
			if lado == 0: #Izquierda
				if room_z > prev_room_z:
					...
					var puerta = preload("res://Escenas/puerta.tscn").instantiate()
					habitacion.add_child(puerta)
					puerta.rotate_y(1.57)
					puerta.set_position(Vector3(room_x,0.3,0))
				...
				if room_x > prev_room_x:
					...
					var puerta = preload("res://Escenas/puerta.tscn").instantiate()
					habitacion.add_child(puerta)
					puerta.set_position(Vector3(0,0.3,room_z))
					puerta.set_name("puerta")
...
func crear_pared_y_meter_a_escena(habitacion: Node3D):
	#Se crea la raiz, la colision y el mesh
	var pared_raiz = StaticBody3D.new()
	var pared_mesh = MeshInstance3D.new()
	var pared_colision = CollisionShape3D.new()
	var colision_colshape = BoxShape3D.new()
...
func agregar_area_pared_inferior(pared_inferior: StaticBody3D):
	var area = Area3D.new()
	var colision_area = CollisionShape3D.new()
	var colshape_colision_area = BoxShape3D.new()
	area.add_child(colision_area)
	colshape_colision_area.set_size(Vector3(2,2,2))
...
func crear_habitacion_de_cuatro_paredes():
	var habitacion: Node3D = Node3D.new()
	...
	var area_col_shape = BoxShape3D.new()
	area.add_child(area_col)
	area_col_shape.set_size(Vector3(2,ancho_pared,2))
	area_col.set_shape(area_col_shape)
	...
	#Pared DE ARRIBA
	var pared_superior: StaticBody3D = crear_pared_y_meter_a_escena(habitacion)
	...
	var pared_izquierda: StaticBody3D = crear_pared_y_meter_a_escena(habitacion)
	...
	var pared_inferior: StaticBody3D = crear_pared_y_meter_a_escena(habitacion)
	...
	var piso: StaticBody3D = crear_pared_y_meter_a_escena(habitacion)
	piso.set_name("piso")
	#Tamaño del piso
	piso.get_child(0).set_surface_override_material(0, piso_material)

	
	var pared_derecha: StaticBody3D = crear_pared_y_meter_a_escena(habitacion)
...
func dar_escala_y_posicion_a_paredes(habitacion):
	...
	#Posicion de la pared de arrib
	habitacion.get_node("pared_superior").set_position(Vector3(0,room_y,-room_z))
	...
	habitacion.get_node("pared_izquierda").set_position(Vector3(-room_x,room_y,0))
	...
	habitacion.get_node("pared_inferior").set_position(Vector3(0,room_y,room_z))
	
	habitacion.get_node("piso").set_scale(Vector3(room_x,ancho_pared,room_z))
	#Posicion del piso
	habitacion.get_node("piso").set_position(Vector3(0,ancho_pared,0))
	...
	habitacion.get_node("pared_derecha").set_position(Vector3(room_x,room_y,0))
